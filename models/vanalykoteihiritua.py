...
## 実テーブル
class VAnalyKoteiHirituA(db.Model): 
    __tablename__ = "v_analy_kotei_hiritu_a"
    ...
    kotei_sisan        = db.Column(db.Numeric, primary_key=False) 
    sihon              = db.Column(db.Numeric, primary_key=False) 
    kotei_hiritu       = db.Column(db.Numeric, primary_key=False) 

class VAnalyKoteiHirituASchema(ma.SQLAlchemyAutoSchema):
      class Meta:
            model = VAnalyKoteiHirituA
            load_instance = True
